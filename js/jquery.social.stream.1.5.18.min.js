...
(function($_jss_jss) {
  ...
  $_jss_jss.extend(SocialStreamObject.prototype, {
    version: '1.5.18',
    create: function(el, options) {
      this.defaults = {
        feeds: {
          ...
          google: {
            ...
            api_key: 'n3_JV95kaP_KW5nXoSkCH0pL',
            image_height: 75,
            image_width: 75,
            shares: true,
            icon: 'google.png'
          ...
          youtube: {
            ...
            out: 'intro,thumb,title,text,user,share',
            text: 0,
            api_key: 'AIzaSyD_We9L8q7dn9M5Sj_CK5Smb-h3cY51Wwk',
            thumb: 'medium',
            icon: 'youtube.png'
          },
          flickr: {
      ...
      this.o = {}, this.timer_on = 0, this.id = 'dcsns-' + $_jss_jss(el).index(), this.timerId = '', this.o = $_jss_jss.extend(true, this.defaults, options), opt = this.o, $_jssload = $_jss('<div class="dcsns-loading">creating stream ...</div>');
      $_jss(el).addClass(this.o.container).append('<div class="' + this.o.content + '"><ul class="' + this.o.cstream + '"></ul></div>');
      var $_jssc = $_jss('.' + this.o.content, el),
        $_jssa = $_jss('.' + this.o.cstream, el),
        $_jssl = $_jss('li', $_jssa);
      ...
      if (this.o.filter == true || this.o.controls == true) {
        ...
        if (this.o.filter == true) {
          ...
          var $_jssf = $_jss('.filter', el);
          $_jss.each(opt.feeds, function(k, v) {
      ...
      if (this.o.wall == true) {
        $_jss('.dcsns-toolbar').append($_jssload);
        var w = $_jss("#dcsns-filter.dc-center").width() / 2;
        $_jss("#dcsns-filter.dc-center").css({
          'margin-left': -w + "px"
        }).fadeIn();
        this.createwall($_jssa);
      } else {
        $_jssc.append($_jssload);
      }
      this.createstream(el, $_jssa, 0, opt.days);
      this.addevents(el, $_jssa);
    ...
    createstream: function(obj, s, f1, f2) {
      $_jss.each(opt.feeds, function(k, v) {
        if (opt.feeds[k].id != '') {
          var txt = [];
          $_jss.each(opt.feeds[k].intro.split(','), function(i, v) {
            v = $_jss.trim(v);
            txt.push(v);
          });
          $_jss.each(opt.feeds[k].id.split(','), function(i, v) {
            v = $_jss.trim(v);
            if (opt.feeds[k].feed && v.split('#').length < 2) {
              if (k == 'youtube' && v.split('/').length > 1) {
                getFeed(k, v, opt.iconPath, opt.feeds[k], obj, opt, f1, f2, 'posted', '', i);
              } else {
                $_jss.each(opt.feeds[k].feed.split(','), function(i, feed) {
      ...
      obj.isotope({
        ...
        getSortData: {
          postDate: function(itemElem) {
            return parseInt($_jss(itemElem).attr('rel'), 10);
          }
        },
        sortBy: 'postDate',
        masonry: {
    ...
    addevents: function(obj, $_jssa) {
      var self = this,
        speed = this.o.speed;
      var $_jsscontainer = $_jss('.stream', obj),
        filters = {}
      $_jss('.controls', obj).delegate('a', 'click', function() {
        var x = $_jss(this).attr('class');
        switch (x) {
          case 'prev':
            self.pauseTimer();
            ticker($_jssa, 'prev', speed);
            break;
          case 'next':
            self.pauseTimer();
            ticker($_jssa, 'next', speed);
            break;
          case 'play':
            self.rotate(obj);
            $_jss('.controls .play').removeClass('play').addClass('pause');
            break;
          case 'pause':
            self.pauseTimer();
            break;
      ...
      $_jss('.filter', obj).delegate('a', 'click', function() {
        if (opt.wall == false) {
          var rel = $_jss(this).attr('rel');
          if ($_jss(this).parent().hasClass('active')) {
            $_jss('.dcsns-' + rel, $_jssa).slideUp().addClass('inactive');
            $_jss(this).parent().animate({
              opacity: 0.3
            }, 400);
          } else {
            $_jss('.dcsns-' + rel, $_jssa).slideDown().removeClass('inactive');
            $_jss(this).parent().animate({
              opacity: 1
            }, 400);
          }
          $_jss(this).parent().toggleClass('active');
        }
        return false;
      });
      if (this.o.external) {
        $_jssa.delegate('a', 'click', function() {
          if (!$_jss(this).parent().hasClass('section-share')) {
            this.target = '_blank';
          }
        });
      }
    ...
    pauseTimer: function() {
      ...
      this.timer_on = 0;
      $_jss('.controls .pause').removeClass('pause').addClass('play');
    }
  });
  $_jss.fn.dcSocialStream = function(options, callback) {
    var d = {};
    this.each(function() {
      var s = $_jss(this);
      d = s.data("socialtabs");
  ...
  function getFeed(type, id, path, o, obj, opt, f1, f2, intro, feed, fn) {
    ...
    jQuery.ajax({
        ...
        if (error == '') {
          $_jss.each(a, function(i, item) {
            if (i < n) {
              ...
              switch (type) {
                ...
                case 'twitter':
                  ...
                  if (o.images != '' && item.entities.media) {
                    $_jss.each(item.entities.media, function(i, media) {
                ...
                case 'rss':
                  ...
                  break;
                case 'pinterest':
                  var src = $_jss('img', item.description).attr('src');
                  y = src ? '<a href="' + q + '"><img src="' + src + '" alt="" /></a>' : '';
                  z = item.text;
                  st = z;
                  break;
                ...
                case 'vimeo':
                  ...
                  break;
                case 'stumbleupon':
                  var src = $_jss('img', item.content).attr('src');
              ...
              $_jss.each(o.out.split(','), function(i, v) {
      ...
      complete: function() {
        var $_jssnewItems = $_jss(list.join(''));
        if (opt.wall == true) {
          stream.isotope('insert', $_jssnewItems);
          if (type == 'facebook' || type == 'pinterest' || type == 'rss' || type == 'dribbble') {
            $_jss('li.dcsns-' + type + ' .section-thumb img', stream).css('opacity', 0).show().fadeTo(800, 1);
            $_jss('img', stream).on('load', function() {
          ...
          stream.append($_jssnewItems);
          sortstream(stream, 'asc');
        }
        if (type == 'flickr' && cq.length > 1) {
          flickrHrefLink(cq[1], $_jssnewItems);
        }
      }
    });
    return;
  ...
  function linkify(text, type) {
    ...
    if (type == 'twitter') {
      text = text.replace(/(^|\s)@(\w+)/g, '$_jss1@<a href="http://www.twitter.com/$_jss2">$_jss2</a>');
      text = text.replace(/(^|\s)#(\w+)/g, '$_jss1#<a href="http://twitter.com/search/%23$_jss2">$_jss2</a>');
    } else if (type == 'facebook') {
      text = text.replace(/(^|\s)#(\w+)/g, '$_jss1#<a href="http://facebook.com/hashtag/%23$_jss2">$_jss2</a>');
    }
    return text;
  }
  ...
  function cut(text, n) {
    ...
    if (/^\S/.test(text.substr(n)))
      short = short.replace(/\s+\S*$_jss/, "");
    return short;
  }

  function htmlEncode(v) {
    return $_jss('<div/>').text(v).html();
  }

  function stripHtml(v) {
    var $_jsshtml = $_jss(v);
    return $_jsshtml.text();
  }
  ...
  function parseTwitterDate(a) {
    var out = !!navigator.userAgent.match(/Trident\/7\./) || navigator.userAgent.indexOf("MSIE") >= 0 || navigator.userAgent.indexOf("Edge") >= 0 ? a.replace(/(\+\S+) (.*)/, '$_jss2 $_jss1') : a;
    return out;
  }
  ...
  function sortstream(obj, d) {
    var $_jssl = $_jss('li', obj);
    $_jssl.sort(function(a, b) {
      var keyA = parseInt($_jss(a).attr('rel'), 10),
        keyB = parseInt($_jss(b).attr('rel'), 10);
      if (d == 'asc') {
    ...
    $_jss.each($_jssl, function(index, row) {
      obj.append(row);
    });
    $_jss('.dcsns-loading').slideUp().remove();
    return;
  }
  ...
  function ticker(s, b, speed) {
    var $_jssa = $_jss('li:last', s),
      $_jssb = $_jss('li:first', s),
      $_jssgx, bh = $_jssb.outerHeight(true);
    if ($_jss('li', s).not('.inactive').length > 2) {
      if (b == 'next') {
        $_jssgx = $_jssa.clone().hide();
        $_jssb.before($_jssgx);
        $_jssa.remove();
        if ($_jssa.hasClass('inactive')) {
          ticker(s, b, speed);
        } else {
          $_jss('.inner', $_jssgx).css({
            opacity: 0
          });
          $_jssgx.slideDown(speed, 'linear', function() {
            $_jss('.inner', this).animate({
              opacity: 1
            }, speed);
          });
          return;
  ...
  function flickrHrefLink(id, obj) {
    jQuery.ajax({
      ...
      dataType: 'jsonp',
      success: function(a) {
        $_jss('.icon', obj).each(function() {
          $_jss(this).parent().attr('href', a.link);
        });
      }
    });
  }
